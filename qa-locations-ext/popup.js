...
function setImportStatus(message, tone = '') {
  ...
  if (tone) {
    importStatus.classList.add(tone);
  }
}
...
async function handlePickedFile(kind, file) {
  if (!file) return;
  try {
    if (kind === 'locations') {
